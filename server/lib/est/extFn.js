...
exports.getAppLink = function(protocol,param , os , r ,wapHref){
	if (!protocol || !param) return false
	if (!os.mlsApp) return wapHref || '###noapp'
	if (r) param.r = r
	var link = 'veryside'
	if (os && os.ipad) link = 'verysidehd'
	link += '://'+ protocol +'.veryside?json_params='+ encodeURIComponent(JSON.stringify(param))
	return link	
	}
