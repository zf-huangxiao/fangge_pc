var commentsPage = 2;
var comPendingRequests={},comTimer;

var $MoreCommentsWrapper = $('.comments-checkMore'),
    $loadMoreConMsg = $('.load-more-comments-msg'),
    $CommentsLoading = $('.comments-loading'),
    $noCommentsData = $('.comments-no-data');
...
var loadMoreComments = {
    ...
    ajaxConfig:function(){
        $.ajaxSettings.beforeSend = function(xhr,options){
            ...
            if( !flag ){
                ...
                if( comPendingRequests[key] !== undefined) {
                    //console.log('阻断');
                    return false;
                    //xhr.abort();
                }
                else{
                    comPendingRequests[key] = xhr;
                    // 放弃后触发的重复提交
                    //comPendingRequests[key].abort(); // 放弃先触发的提交
                }
                comTimer = setTimeout(function(){
                    comTimer = null;
                    //console.dir(comPendingRequests);
                    comPendingRequests = {};
                },5000);
    ...
    initLoadActive : function(){
        $MoreCommentsWrapper.find('.msg').removeClass('active');
         $loadMoreConMsg.addClass('active');
    },

    loadlingActive : function(){
        $MoreCommentsWrapper.find('.msg').removeClass('active');
        $CommentsLoading.addClass('active');
    },

    noDataActive : function(){
        $MoreCommentsWrapper.find('.msg').removeClass('active');
        $noCommentsData.addClass('active');
    },
